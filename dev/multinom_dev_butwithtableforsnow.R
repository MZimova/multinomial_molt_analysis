  # Under construction!
  # Trying to add fixed effect of snow 
...
################################################################################
    ...
    setwd("/Users/marketzimova/Documents/WORK/DISSERTATION/3 Camera Traps Study/GitHub/SSH-camera-trap-analysis-MZ_branch2")
...
################################################################################
    ...
    h2 <- hares %>%
      mutate(
        ...
        cam = as.numeric(as.factor(CameraNum)),
        first_day = min(days),
        last_day = max(days),
        response = as.integer(cut(hares$White3, 3, labels = 1:3))
      )
      
    snow <- h2 %>%
      select(cam, days, Snow) %>%
      group_by(cam, days) %>% 
      summarise(Snow = mean(Snow, na.rm = T)) %>% 
      ungroup %>% 
      tidyr::spread(days, Snow)
    ...
    #response <- cut(hares$White3, 3, labels = c(0,50,100))
    #response <- cut(hares$White, 5, labels = c(0,25,50,75,100))
    ...
    parms <- c(
      "p_rand", "pp", "beta", "alpha", "sig_cam", "tau_cam", "elev_eff"
    )
    ...
    out <- jags(
      ...
      model.file = "models/multinom_randCam_onalllines_3_covs.txt", 
      n.chains = 3,
      n.iter = 1000,
      n.burnin = 500,
      n.thin = 3 
