...
hares <- select(hares_cl, Site, White3, White, Julian, Cluster, Year, fCluster, Camera) %>%
  subset(Julian >50 & Julian <200 & Site=="Can") # & Year==2015
# if running with temp remove CO_68: it is missing daymet! 
#hares <-filter(hares,Cluster != 68)
fCluster_count<- length(unique(hares$fCluster))
...
# Run with cameras as random:
#camera_names <- unique(hares$Camera); cameras <- filter(cameras, Camera %in% camera_names)
cameras_all<-cameras

# Run with clusters as random:
camera_names <- unique(hares$fCluster)
cameras <- filter(cameras, fCluster %in% camera_names)
fCluster_count<-length(unique(cameras$fCluster))
...
days <- as.integer(unlist(hares[,time_scale])) #; length(days)
(first_day <- min(days))
(last_day <- max(days))
...
spread_daymet <- daymet %>%
  ...
  summarise(tmax = mean(tmax, na.rm = T)) %>% #if not working on camera basis
  ungroup %>% 
...
dat <- list(
  nobs = nrow(hares),
  day = days, 
  cam = as.numeric(as.factor(hares$ClusterYr)), # for non-daily use: hares$Cluster
  y = hares$response,
  nbins = 3,
  ndays = last_day,
  ncam = length(unique(hares$ClusterYr)), # for non-daily use: hares$Cluster
  dtemp = spread_daymet
  #elev = as.numeric(GeoCL$ElevSC),
  #tavg30 = as.numeric(GeoCL$LatSC)
  #tavg30 = as.numeric(past_daymet$tmin30SC)
...
out <- jags.parallel(
  ...
  model.file = "/Users/marketzimova/Documents/WORK/DISSERTATION/GitHub/multinomial_molt_analysis/models/multinom_covs.txt", #multinom_covs2 or multinom
  n.chains = 3,
  n.iter = 5000,
  n.burnin = 2000,
  n.thin = 3
