...
jags_call <- function(x, time_scale, ...){
  ...
  inits <- function(){
    list(
      ...
      tau = 5,
      start = 100,
      tauChange= 5 #added for camera random effect
    ) 
  }
  ...
  dat <- list(
    nobs = nrow(x),
    cam = as.numeric(as.factor(x$Camera)),
    day = as.numeric(unlist(x[,time_scale])), 
    white = x$White,
    ncam = length(unique(x$Camera))
  )
  
  # Parameters to monitor
  parms <- c("alpha", "beta", "beta1", "tau", "start", "end", "tauChange")
